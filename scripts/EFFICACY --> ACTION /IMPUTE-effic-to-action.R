...
df_cr <- tab_in %>%
  ...
  dplyr::filter(Wave == 2013) %>%
  dplyr::filter(!is.na(Env.Eff01.ActionBelief)) %>%
  dplyr::filter(!is.na( Env.Eff02.ActionFeeling)) %>%
  dplyr::filter(!is.na(Env.Eff01.ActionBelief_lead1)) %>%
  dplyr::filter(!is.na( Env.Eff02.ActionFeeling_lead1)) %>%
...
saveh(mice_cc, "environ_5_efficacy")


mice_cc <- readh("environ_5_efficacy")
...
saveh(data_imputed, "ml_environ_5_efficacy")
#data_imputed <- readh("ml_environ_5_efficacy")
