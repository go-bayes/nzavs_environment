...
# hist(data_long$Env.ClimateChgConcern_lead1)
# sd(data_long$Env.ClimateChgConcern_lead1)
...
h_tab |>
  kbl(caption = main,
      ...
      "html") |>
  kable_styling() %>%
  row_spec(c(1:6),  # Bold out the lines where EVALUES do not cross zero or for ratios, 1
           bold = T,
           # color = "black",
           background = "bold") |>
  kable_minimal(full_width = F)
...
h_tab |>
  kbl(caption = main,
      ...
      "html") |>
  #kable_styling() %>%
  row_spec(c(1,4),  # Bold out the lines where EVALUES do not cross zero or for ratios, 1
           bold = T,
           # color = "black",
           background = "bold") |>
  kable_minimal(full_width = F)
...
sacrificemade4_p +
sacrificenorms4_p +
  plot_annotation(title = "Causal effects of environmental concern on stated environmental values") +
...
motorway5_p +
publictransport5_p +
  plot_annotation(title = "Causal effects of environmental concern on revealed environmental values") +
  plot_layout(guides = 'collect')
