...
## tables 
table1::table1( ~ Env.ClimateChgReal | Wave,
        data = dom,
        transpose = T)  # ALL
table1( ~ Env.ClimateChgCause |
          Wave,
        data = dom,
        transpose = T)  # ALL
table1( ~ EnvEfficacy |
          Wave, data = dom, transpose = T)  # No 15 or 18
table1( ~ Env.SacMade |
          Wave, data = dom, transpose = T)  # No 15 No 16 No 18
table1( ~ Env.SatNZEnvironment |
          Wave,
        data = dom,
        transpose = T) # all   # No 15 No 16 No 18
table1( ~ Your_Future_Security |
          Wave,
        data = dom,
        transpose = T) # all  All
table1( ~ KESSLER6 | Wave, data = dom, transpose = T) # all  09
table1( ~ Env.ClimateChgConcern |
          Wave,
        data = dom,
        transpose = T) # Missing first 4 years
table1( ~ Env.ClimateChgConcern |
          factor(yearsM),
        data = dom,
        transpose = T)
...
mt <-
  lmer(Env.ClimateChgConcern ~ years * (Your_Future_SecurityS + K6S) + (1 |
       ...
       data = dom)
mtt <-
  lmer(Env.ClimateChgConcern ~ years * (RelidS) + (1 |
                                                      Id), data = dom)
mttp <-
  lmer(Env.ClimateChgConcern ~ yearsM * (Pol.OrientS) + (1 |
                                                           Id), data = dom)

na <-
  lmer(Env.PubTransSubs ~ years * (Env.ClimateChgConcernS) + (1 |Id),
       data = dom)
summary(na)
nb <-
  lmer(Env.MotorwaySpend ~ years * (Env.ClimateChgConcernS) + (1 |
                                                                  Id),
       data = dom)
nc <-
  lmer(Env.NATIVE.SPECIES ~ years * (Env.ClimateChgConcernS) + (1 |
                                                                   Id),
       data = dom)

na1 <-
  lmer(Env.PubTransSubs ~ years * (Env.SatNZEnvironmentS) + (1 |
                                                               Id),
       data = dom)

nr <-
  lmer(Env.MotorwaySpend ~ years * (Env.SatNZEnvironmentS) + (1 |
                                                                Id),
       data = dom)
np <-
  lmer(Env.NATIVE.SPECIES ~ years * (Env.SatNZEnvironmentS) + (1 |
                                                                 Id),
       data = dom)
...
plot(ggpredict(na, terms = c(
  "years", "Env.ClimateChgConcernS [minmax]"
)))

plot(ggpredict(nb, terms = c(
  "yearsM", "Env.ClimateChgConcernS [minmax]"
)))

plot(ggpredict(nc, terms = c(
  "yearsM", "Env.ClimateChgConcernS [minmax]"
)))


plot(ggpredict(na1, terms = c(
  "years", "Env.SatNZEnvironmentS [minmax]"
)))
plot(ggpredict(nr, terms = c(
  "years", "Env.SatNZEnvironmentS [minmax]"
)))
plot(ggpredict(np, terms = c(
  "years", "Env.SatNZEnvironmentS [minmax]"
)))
...
str(dom$Wave)
ml <-
  lmer(Env.SacMade ~ years * (Env.SatNZEnvironmentS * RelixdS) + (Id |
                                                                    Wave),
       data = dom)
mlPP <-
  lmer(Env.SacMade ~ years * (Env.SatNZEnvironmentS * Pol.OrientS) + (1 |
                                                                        Id),
       data = dom)
mlk <-
  lmer(Env.SacMade ~ years * (Your_Future_SecurityS * RelidS) + (1 |
                                                                   Id),
       data = dom)
mlkp <-
  lmer(Env.SacMade ~ years * (Your_Future_SecurityS * Pol.OrientS) + (1 |
                                                                        Id),
       data = dom)
...
mll <-
  lmer(Env.SacMade ~ years * (Env.ClimateChgConcernS * RelidS) + (1 |
                                                                    Id),
       data = dom)
mlp <- lmer(Env.SacMade ~ years * (EnvEfficacyS) + (1 |
                                                      Id), data = dom)
mld <-
  lmer(EnvEfficacy ~ years * (Env.ClimateChgConcernS) + (1 |
                                                           Id), data = dom)
mld <-
  lmer(EnvEfficacy ~ years * (Env.ClimateChgConcernS) + (1 |
                                                           Id), data = dom)
mls <-
  lmer(EnvEfficacy ~ years * (Env.SatNZEnvironmentS) + (1 |
                                                          Id), data = dom)
mla <-
  lmer(Env.SacMade ~ years * (Your_Future_SecurityS) + (1 |
                                                          Id), data = dom)
...
mk <- lmer(Env.ClimateChgConcern ~ yearsM * K6S + (1 |
                                                     Id), data = domM)
...
plot(ggpredict(
  ml,
  terms = c("years", "Env.SatNZEnvironmentS [minmax]", "RelidS")
)) # USE
plot(ggpredict(mlPP, terms = c("years", "Env.SatNZEnvironmentS"))) # USE
plot(ggpredict(mlPP, terms = c(
  "years", "Env.SatNZEnvironmentS [minmax]"
))) # USE

plot(ggpredict(
  mlk,
  terms = c("years", "Your_Future_SecurityS [minmax]", "RelidS")
))  # NOT FUTURE SECURITY BUT SATISFACTION WITH ENVIRONMENT
plot(ggpredict(
  mlkp,
  terms = c("years", "Your_Future_SecurityS [minmax]", "Pol.OrientS")
))  # NOT FUTURE SECURITY BUT SATISFACTION WITH ENVIRONMENT  ## WOW
...
plot(ggpredict(mld, terms = c(
  "years", "Env.ClimateChgConcernS[minmax]"
))) # linear on religion USE
plot(ggpredict(mld, terms = c(
  "years", "Env.ClimateChgConcernS[minmax]"
))) # linear on religion # USE
plot(ggpredict(mls, terms = c(
  "years", "Env.SatNZEnvironmentS[minmax]"
))) # USE ** CROSS'
#then
#
plot(ggpredict(mla, terms = c(
  "years", "Your_Future_SecurityS[minmax]"
))) # USE ** CROSS
...
table1::table1( ~ Env.SacWilling + Env.SacMade |
                  Wave * Relid,
                data = dom,
                overall = F)

table1::table1(
  ~ Env.SacWilling + Env.SacMade |
    Wave ,
  data = dom,
  overall = F,
  transpose = T
...
table1::table1(
  ~ Age + Edu + Employed  + Male + Pol.Orient + Relid + Urban |
    Wave,
  data = dom,
  overall = F,
  droplevels = FALSE
...
KESSLER6 <- BGW %>%
  ...
  dplyr::mutate(KESSLER6_CAT = cut(
    KESSLER6,
    breaks = c(-Inf, 5, 13, Inf),
    labels = c("low_anxiety", "middle_anxiety", "high_anxiety")
  )) %>%
  dplyr::mutate(KESSLER6_Bin = cut(
    KESSLER6,
    breaks = c(-Inf, 13, Inf),
    labels = c("low_anxiety", "high_anxiety")
  ))
...
library(lcsm)
plot_trajectories(
  data = Env.ClimateChgCause,
  id_var = "Id",
  var_list = x_var_list,
  xlab = "Time",
  ylab = "Env.ClimateChgCause",
  connect_missing = F,
  random_sample_frac = .015,
  title_n = TRUE
) + facet_wrap( ~ Id)
...
table1::label(dom$Env.SacWilling) <-
  "Willing to Sacrifice Environment"
dom$Religious <-
  factor(dom$Male, labels = c("Not Religious", "Religious"))
library("tidyLPA")
library("tidyverse")
# library("dplyr")
#
...
ggplot(data = df, aes(x = ds, y = y)) +
  ...
  labs(
    title = "Total daily precipitation in Boulder, Colorado",
    subtitle = "Fall 2013",
    x = "Date",
...
#am.out<-am.out.dom
domALL <- transform.amelia(
  am.out.dom,
  Id = factor(Id),
  Age.C.decade = scale(Age, center = TRUE, scale =
                         FALSE) / 10,
  Male = factor(Male),
  Pol.Orient.C = scale(Pol.Orient, center = T, scale =
                         F),
  Relid.C = scale(Relid,  center = T, scale = F),
  Pol.Orient.S = scale(Pol.Orient, center = T, scale =
                         T),
...
forlatex <-
  lmer(
    Env.ClimateChgConcern ~  years * (Relid.S + Pol.Orient.S + Edu.S)  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  + (1 |
                                                                                                                                   Id),
    data = domALL$imputations$imp1
  )
...
for (i in 1:m) {
  plots[[i]] <-
    plot(out[[i]], facets = TRUE, color = "viridis") + scale_y_continuous(limits =
                                                                            c(4, 6))
}
...
for (i in 1:m) {
  out[[i]] <-
    ggpredict(models[[i]], terms = c("EthnicCats", "years [4,6.5,9]"))
}
m <- 10
plots <- NULL
for (i in 1:m) {
  plots[[i]] <-
    plot(out[[i]], facets = T, color = "viridis")  + scale_y_continuous(limits =
                                                                          c(4, 6))
}
# Inspect all  # No differences
plots[[1]]
...
for (i in 1:m) {
  out[[i]] <-
    ggpredict(models[[i]], terms = c("Edu.S", "years [4,6.5,9]"))
}
m <- 10
plots <- NULL
for (i in 1:m) {
...
for (i in 1:m) {
  out[[i]] <-
    ggpredict(models[[i]], terms = c("Pol.Orient.S", "years [4,6.5,9]"))
}
m <- 10
plots <- NULL
for (i in 1:m) {
...
for (i in 1:m) {
  out[[i]] <-
    ggpredict(models[[i]], terms = c("Relid.S [minmax]", "years [4,6.5,9]"))
}
m <- 10
plots <- NULL
for (i in 1:m) {
...
for (i in 1:m) {
  models[[i]] <-
    lmer(
      Env.ClimateChgReal ~  years * (Relid.S + Pol.Orient.S + Edu.S)  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  + (1 |
                                                                                                                                  Id),
      data = domALL$imputations[[i]]
    )
}
...
for (i in 1:m) {
  out[[i]] <-
    ggpredict(models[[i]], terms = c("EthnicCats", "years [0,4.5,9]"))
}
m <- 10
plots <- NULL
for (i in 1:m) {
...
for (i in 1:m) {
  out[[i]] <-
    ggpredict(models[[i]], terms = c("Edu.S", "years [0,4.5,9]"))
}
m <- 10
plots <- NULL
for (i in 1:m) {
...
for (i in 1:m) {
  out[[i]] <-
    ggpredict(models[[i]], terms = c("Pol.Orient.S", "years [0,4.5,9]"))
}
m <- 10
plots <- NULL
for (i in 1:m) {
...
for (i in 1:m) {
  out[[i]] <-
    ggpredict(models[[i]], terms = c("Relid.S [minmax]", "years [0,4.5,9]"))
}
m <- 10
plots <- NULL
for (i in 1:m) {
...
for (i in 1:m) {
  models[[i]] <-
    lmer(
      Env.ClimateChgCause ~  years * (Relid.S + Pol.Orient.S + Edu.S)  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  + (1 |
                                                                                                                                   Id),
      data = domALL$imputations[[i]]
    )
}
...
for (i in 1:m) {
  models[[i]] <-
    lmer(
      EnvEfficacy ~  years * (Relid.S + Pol.Orient.S + Edu.S)  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  + (1 |
                                                                                                                           Id),
      data = domALL$imputations[[i]]
    )
}
...
for (i in 1:m) {
  models[[i]] <-
    lmer(
      Env.SacMade ~  years * (Relid.S + Pol.Orient.S + Edu.S)  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  + (1 |
                                                                                                                           Id),
      data = domALL$imputations[[i]]
    )
}
...
for (i in 1:m) {
  models[[i]] <-
    lmer(
      Env.SacMade ~ years * (Env.SatNZEnvironmentS * Relid.S) + Pol.Orient.S + Edu.S  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  +  (1 |
                                                                                                                                                   Id),
      data = domALL$imputations[[i]]
    )
}
...
library(ggthemes)
X_SACRIFICEMADE_Env.SatNZEnvironmentS_Relid.S <-
  plots[[10]]   + theme_clean()
...
for (i in 1:m) {
  models[[i]] <-
    lmer(
      Env.SacMade ~ years * (Env.SatNZEnvironmentS * Pol.Orient.S) + Relid.S + Edu.S  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  +  (1 |
                                                                                                                                                   Id),
      data = domALL$imputations[[i]]
    )
}
...
library(ggthemes)
X_SACRIFICEMADE_Env.SatNZEnvironmentS_Pol.Orient.S <-
  plots[[10]]   + theme_clean()
...
for (i in 1:m) {
  models[[i]] <-
    lmer(
      Env.SacMade ~ years * (Your_Future_SecurityS * Relid.S) + Pol.Orient.S + Edu.S  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  +  (1 |
                                                                                                                                                   Id),
      data = domALL$imputations[[i]]
    )
}
...
library(ggthemes)
X_SACRIFICEMADE_Your_Future_SecurityS_Relid.S <-
  plots[[10]]   + theme_clean()
...
for (i in 1:m) {
  models[[i]] <-
    lmer(
      Env.SacMade ~ years * (Your_Future_SecurityS * Pol.Orient.S) + Relid.S + Edu.S  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  +  (1 |
                                                                                                                                                   Id),
      data = domALL$imputations[[i]]
    )
}
...
library(ggthemes)
X_SACRIFICEMADE_Your_Future_SecurityS_Pol.Orient.S <-
  plots[[10]]   + theme_clean()
...
for (i in 1:m) {
  models[[i]] <-
    lmer(
      Env.PubTransSubs ~ years * (Env.ClimateChgConcernS) + Pol.Orient.S + Relid.S + Edu.S  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  + (1 |
                                                                                                                                                        Id),
      data = domALL$imputations[[i]]
    )
}
...
for (i in 1:m) {
  out[[i]] <-
    ggpredict(models[[i]],
              terms = c("years [4,6.5,9]", "Env.ClimateChgConcernS [meansd]"))
}
m <- 10
plots <- NULL
for (i in 1:m) {
...
library(ggthemes)
XX_PLOT_PubTransSubsEnv.ClimateChgConcernS <-
  plots[[10]]   + theme_clean()
...
for (i in 1:m) {
  models[[i]] <-
    lmer(
      Env.MotorwaySpend ~ years * (Env.ClimateChgConcernS) + Pol.Orient.S + Relid.S + Edu.S  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  +  (1 |
                                                                                                                                                          Id),
      data = domALL$imputations[[i]]
    )
}
...
library(ggthemes)
XX_PLOT_Env.MotorwaySpend.ENVCLIMATECONCERN <-
  plots[[10]]   + theme_clean()
...
for (i in 1:m) {
  models[[i]] <-
    lmer(
      Env.NATIVE.SPECIES ~ years * (Env.ClimateChgConcernS) + Pol.Orient.S + Relid.S + Edu.S  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  +  (1 |
                                                                                                                                                           Id),
      data = domALL$imputations[[i]]
    )
}
...
library(ggthemes)
XX_PLOT_Env.NATIVE.SPECIES.ENVCONCERN <-
  plots[[10]]   + theme_clean()
...
for (i in 1:m) {
  models[[i]] <-
    lmer(
      Env.PubTransSubs ~ years * (Env.SatNZEnvironmentS) + Pol.Orient.S + Relid.S + Edu.S  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  +  (1 |
                                                                                                                                                        Id),
      data = domALL$imputations[[i]]
    )
}
...
for (i in 1:m) {
  out[[i]] <-
    ggpredict(models[[i]],
              terms = c("years [0,4.5,9]", "Env.SatNZEnvironmentS [meansd]"))
}
m <- 10
plots <- NULL
for (i in 1:m) {
...
for (i in 1:m) {
  models[[i]] <-
    lmer(
      Env.MotorwaySpend ~ years * (Env.SatNZEnvironmentS) + Pol.Orient.S + Relid.S + Edu.S  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  +  (1 |
                                                                                                                                                         Id),
      data = domALL$imputations[[i]]
    )
}
...
library(ggthemes)
XY_PLOT_Env.MotorwaySpend.SATENVIRON <-
  plots[[10]]   + theme_clean()
...
for (i in 1:m) {
  models[[i]] <-
    lmer(
      Env.NATIVE.SPECIES ~ years * (Env.SatNZEnvironmentS) + Pol.Orient.S + Relid.S + Edu.S  + Age.C.decade  + EthnicCats  + Male + NZdepS + Urban  +  (1 |
                                                                                                                                                          Id),
      data = domALL$imputations[[i]]
    )
}
...
XY_PLOT_Env.NATIVE.SPECIES.SATENVIRON <-
  plots[[10]]   + theme_clean()
...
library(patchwork)
(plot0 / (plot1 | plot2))  + plot_annotation(
  tag_levels = 'a',
  title = 'T',
  subtitle = '',
  caption = '',
...
plot(pr2,
     facet = T,
     ci.style = "errorbar",
     colors = "bw") + scale_y_continuous(limits = c(3, 7))
...
plot(pr5.1, facet = F, use.theme = F) + theme_linedraw() + theme(plot.title = element_text(size =
                                                                                             10)) +
  scale_y_continuous(limits = c(1, 7)) +
  scale_x_continuous(
    labels = c(
  ...
  labs(
    x = "years: 2009 - 2017",
    y = "Sacrifice Made for the Environment: 1-7",
    title = "Predicted values of Sacrifices Made the Environment in New Zealand 2009 -2017(N = 31,464)",
    colour = get_legend_title(pr5.1)
  )
...
plot(pr6, facet = F, use.theme = F) + theme_linedraw() + theme(plot.title = element_text(size =
                                                                                           10)) +
  scale_x_continuous(
    labels = c(
      "2009",
  ...
  labs(
    ...
    title = "Predicted values of Nationalism in New Zealand 2009 -2017, N = 31,464)",
    colour = get_legend_title(pr5.1)
  )
...
plot(pr7, facet = F, use.theme = F) + theme_linedraw() + theme(plot.title = element_text(size =
                                                                                           10)) +
  scale_x_continuous(
    labels = c(
      "2009",
  ...
  labs(
    ...
    title = "Predicted values of Patriotism in New Zealand 2009 -2017, N = 31,464)",
    colour = get_legend_title(pr7)
  )
...
plot(pr5.1, facet = F, use.theme = F) + theme_linedraw() + theme(plot.title = element_text(size =
  ...
  labs(
    x = "years: 2009 - 2017",
    y = "Sacrifice Made for the Environment: 1-7",
    title = "Predicted values of Sacrifice Made the Environment in New Zealand 2009 -2017 (years =  0 - 8, N = 31,464)",
    colour = get_legend_title(pr5.1)
  )
